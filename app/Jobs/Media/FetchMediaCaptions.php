...
class FetchMediaCaptions implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(
        CaptionStorage $storage,
        SubdlCaptionProvider $subdl,
        OpenSubtitlesCaptionProvider $openSubtitles,
        BoundedMediaDownloader $downloader,
    ): void {
        ...
        $maximumTracks = min(
            100,
            max(1, (int) config('odissey.caption_max_tracks_per_item', 20)),
        );
        foreach ([$subdl, $openSubtitles] as $provider) {
            try {
                foreach ($provider->search($item, $languages) as $candidate) {
                    if ($item->subtitles()->count() >= $maximumTracks) {
                        return;
                    }
                    ...
                    $download = $downloader->download(
                        ...
                        maxBytes: min(
                            ...
                            max(
                                1,
                                (int) config('odissey.caption_max_bytes'),
                            ),
                        ),
                        allowedHost: fn (string $host): bool => $candidate->provider === 'subdl'
                            ? $host === 'dl.subdl.com'
                            : $host === 'opensubtitles.com'
                                || str_ends_with($host, '.opensubtitles.com')
                                || str_ends_with($host, '.opensubtitles.org'),
                        headers: $candidate->headers,
                        maxRedirects: 2,
                        timeoutSeconds: 30,
                    );
                    $path = $storage->store($item, $candidate, $download['body']);
